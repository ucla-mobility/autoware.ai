...
if [[ ! -z "$ROS1_PACKAGES$ROS2_PACKAGES" ]]; then
    ...
    AUTOWARE_COMPILE_WITH_CUDA=1 colcon build --executor parallel --build-base build_ros1 --install-base /opt/autoware.ai/ros/install --cmake-args -DCMAKE_BUILD_TYPE=Release -DCMAKE_LIBRARY_PATH=/usr/local/cuda/lib64/stubs -DCMAKE_CXX_FLAGS=-Wall -DCMAKE_C_FLAGS=-Wall
fi
